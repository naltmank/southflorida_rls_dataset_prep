...
parsed <- tibble(band = band_names) %>%
  mutate(
    zooc = str_extract(band, "^[^_]+"),  # everything before first "_" is the zooplankton
...
times <- terra::time(zooc)
parsed$date <- times

# extract zooc data for buffer region within each band
ext_raw <- terra::extract(zooc, buffers_ll, fun = mean, na.rm = TRUE)
...
ext_long <- ext_raw %>%
  ...
  select(site_code, zooc, depth, date, value)
...
annual_depth_means <- ext_long %>%
  mutate(date = date,
         year = year(date)) %>%
  group_by(site_code, zooc, depth, year) %>%
  summarise(
...
zooc_annual <- annual_depth_means %>%
...
#### INFER MISSING ZOOPLANKTON VALUES ####
...
# replace the standard zooc_mean values with the regional values by habitat
for (v in zooc_vars) {
...
# recombine data
zooc_recombined <- bind_rows(has_depth_rows, dat_na_filled)
...
land_rows <- zooc_recombined %>% 
  filter(if_any(all_of(zooc_vars), is.na))

sea_rows <- zooc_recombined %>% 
  filter(if_all(all_of(zooc_vars), ~ !is.na(.x)))
...
# recombine data
zooc_final <- bind_rows(land_filled, sea_rows)
